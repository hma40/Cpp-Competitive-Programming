...
struct Dinic {
    ...
    vector<pair<int, int>> getMinCutEdges() {
        ...
        for (int u = 0; u < N; ++u) {
            if (lev[u] == -1) continue; // only check from set S
            for (int idx : adj[u]) {
                if (idx & 1) continue; // skip backward edges (they're odd)
                const Edge& e = eds[idx];
...
signed main() {
    ...
    vt<string> v(n);
    F0R(i, n) cin >> v[i];
    F0R(i, n) d.ae(0, i+1, 1);
    F0R(i, n) d.ae(i+n+1, 2*n+1, 1);
    F0R(i, n) F0R(j, n) if(v[i][j]=='o') d.ae(i+1, j+n+1, 1e9);
    int ans = d.maxFlow(0, n*2+1);
    vt<set<pair<int,int>>> adj(n*2+2);

    for (int i = 0; i < d.eds.size(); i += 2) {
        const auto& e = d.eds[i];
        if(e.flo)
        adj[d.eds[i^1].to].insert({e.to, e.flo});
        // if (e.cap > 0) { // only consider forward edges
            // cout << "Flow from " << d.eds[i ^ 1].to << " to " << e.to 
                //  << ": " << e.flo << "/" << e.cap << "\n";
        // }
    }
    // cout << adj << endl;
    cout << ans << endl;
    trav(x, d.getMinCutEdges()) {
        if(x.f==0) {
            cout << "1 " << x.s << endl;
        } else {
            cout << "2 " << x.f-n << endl;
