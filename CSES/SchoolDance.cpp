...
signed main() {
    ...
    int ans = d.maxFlow(0, n+m+1);
    vt<set<pair<int,int>>> adj(n+m+2);
    ...
    F0R(ia, ans) {
        vt<int> path;
        path.add(0);
        while(path.back()!=n+m+1) {
            assert(adj[path.back()].size());
            path.add((*adj[path.back()].begin()).f);
            adj[path[path.size()-2]].erase(adj[path[path.size()-2]].begin());
        }
        cout << path[1] << " " << path[2]-n << endl;
        // cout << endl;
    }
