...
signed main() {
    ...
    cin.tie(0);
    // freopen("steeple.txt" , "r" , stdin);
    // freopen("steeple.txt" , "w", stdout);
    int n;
    cin >> n;
    vt<array<int,3>> vert, horiz;
    F0R(i, n) {
        int a,b,c,d;
        cin >> a >> b >> c >> d;
        if(a==c) {
    ...
    F0R(i, horiz.size()) {
        F0R(j, vert.size()) {
            if(horiz[i][1]<=vert[i][0] && horiz[i][2]>=vert[i][0] && vert[i][1]<=horiz[i][0] && vert[i][2]>=horiz[i][0]) {
                adj.add({i+1, j+horiz.size()+1});
            }
        }
    }
    // cout << adj << endl;
    Match m(horiz.size(), vert.size(), n+1, adj);
    cout << n-m.hopcroft_karp() << endl;
    return 0;
}