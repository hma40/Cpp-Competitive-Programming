...
signed main() {
    ...
    while(t--) {
        ...
        F0R(i, m) cin >> a[i];
        int ones = 0;
        F0R(i, m) if(a[i]==1) ones++;
        int mx = 0;
        F0R(i, m) mx=max(mx, a[i]);
        int mxCount = 0;
        F0R(i, m) if(a[i]==mx) mxCount++;
        if(ones>1 || mxCount>1 || a.back()!=mx) cout << 1 << endl;
        else if(a.back()==mx && mxCount==1) cout << n-mx+1 << endl;
        else cout << n-mx << endl;
    }
    return 0;
}