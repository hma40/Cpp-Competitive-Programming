...
signed main() {
    ...
    while(t--) {
        ...
        FOR(i, 1, n+1) {
            int bad_bef = sum(0, i-1);
            int bad_aft = sum(i+1, 4*i-1);
            if(3*i<=n) bad_aft-=f[3*i];
            if(2*i<=n) bad_aft-=f[2*i];
            // cout << i << " " << bad_aft << " " << bad_bef << endl;
            if(bad_bef+bad_aft>k) continue;
            ans=i;
        }
        cout << ans << endl;
    }
