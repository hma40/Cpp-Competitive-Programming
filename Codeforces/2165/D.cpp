...
signed main() {
    ...
    while(t--) {
        ...
        vt<bool> matchedRight(n);
        vt<bool> matchedLeft(n);
        int ans = n;
        set<int> on;
        auto enable = [&](int i)->void{
            trav(x, inv[i]) {
                if(!matchedRight[x]) on.insert(x);
            }
        };
        auto disable = [&](int i)->void{
            trav(x, inv[i]) on.erase(x);
        };
        FOR(i, 1, 2*n+1) {
            enable(i-1);
            trav(x, inv[i]) {
                auto ub = on.upper_bound(x);
                if(ub==on.begin()) continue;
                --ub;
                // cout << "line 151: matching " << *
                matchedRight[*ub]=matchedLeft[x]=true;
                on.erase(*ub);
                ans--;
            }
            disable(i-1);
