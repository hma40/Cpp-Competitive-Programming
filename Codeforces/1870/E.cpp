...
signed main() {
    ...
    while(t--) {
        ...
        // assert(false);
        vt<vt<int>> exactly(n, vt<int>(n+1, inf));
        R0F(i, n) {
            FOR(j, i, n) {
                // cout << mex[i][j] << endl;
                exactly[i][mex[i][j]]=min(exactly[i][mex[i][j]], j+1);
            }
            if(i!=n-1)
            F0R(j, n+1) {
                exactly[i][j]=min(exactly[i][j], exactly[i+1][j]);
            }
        }
        // cout << exactly << endl;
        // F0R(i, n) F0R(j, n+1) {
            // if(exactly[i][j]!=inf) cout << i << " " << j << " " << exactly[i][j] << endl;
        // }
        // return 0;
        vt<int> first(2*n+1, 1e9);
        first[0]=0;
        ...
        while(pq.size()) {
            ...
            if(first[tp.s]!=tp.f) continue;
            F0R(i, n+1) {
                int nw = tp.s^i;
                int loc = exactly[tp.f][i];
                if(first[nw]>loc) {
                    first[nw]=loc;
                    pq.push({loc, nw});
                }    
        ...
        int ans = 0;
        F0R(i, 2*n+1) if(first[i]<inf) ans=i;
        cout << ans << endl;
    }
    return 0;
}