...
struct SegTree {
    ...
    int rangeQuery2(int index, int left, int right) {
        if(beg[index]==left&&end[index]==right) return tree2[index];
        int ans = none;
        if(beg[2*index]<=right&&end[2*index]>=left) {
            ans=rangeQuery2(2*index, left, min(right, end[2*index]));
        }
        if(beg[2*index+1]<=right&&end[2*index+1]>=left) {
            int r = rangeQuery2(2*index+1, max(left, beg[2*index+1]), right);
            if(ans==none) {
                ans=r;
            } else {
                ans=combi(ans, r);
...
signed main() {
    ...
    while(t--) {
        ...
        vt<int> ans(n);
        F0R(i, 2*n) {
            queries.push({pref[i], -i});
        }
        while(queries.size()) {
            auto first = queries.top();
            queries.pop();
            first.s*=-1;
            if(first.s<n) {
                ans[first.s]=st.rangeQuery(first.s,first.s+n)-st.rangeQuery2(1, first.s, first.s+n)*first.s;
            } 
            st.pointSet(first.s, first.s);
        }
        F0R(i, n) cout << ans[i] << " ";
        cout << endl;
        // cout << pref << endl;
