...
signed main() {
    ...
    while(t--) {
        ...
        F0R(i, m) {
            int a,b,p;
            cin >> a >> b >> p;
            ans+=k*p*tm.inv(tm.nck(n,2));
            ans%=mod;
        }
        ans+=m*tm.nck(k,2)*tm.inv(tm.bexpo(tm.nck(n,2),2));
        ans%=mod;
        cout << ans << endl;
    }
    return 0;
...
/*
probability two students get selected: 1/(nc2)
probability two students don't both get selected: (nc2-1)/(nc2)
P(1) * m + P(2) * (2m+1) + P(3) * (3m+3) + P(4) * (4m+6) + ...

Deal with repeats later?
EV of double ups: 
(kc2)/(nc2)^2*m
*/