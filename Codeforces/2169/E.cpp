...
signed main() {
    ...
    while(t--) {
        ...
        F0R(i, n) cin >> a[i][1];
        F0R(i, n) cin >> a[i][2];
        vt<vt<int>> dp(n+1, vt<int>(16, -inf));
        dp[0][0]=0;
        F0R(i, n) {
            F0R(nwMask, 16) {
                F0R(oldMask, 16) {
                    if((nwMask & oldMask)!=oldMask) continue;
                    // if(nwMask==oldMask) continue;
                    if(dp[i][oldMask]==-inf) continue;
                    if(nwMask==oldMask) {
                        dp[i+1][nwMask]=max(dp[i+1][nwMask], dp[i][oldMask]+a[i][2]);
                        continue;
                    }
                    if(i==1 && oldMask == 6 && nwMask == 15) {
                        // cout << "line 143 " << endl;
                    }
                    int added = nwMask^oldMask;
                    int fuck = dp[i][oldMask];
                    if(added&1) {
                        fuck-=2*a[i][0];
                    }
                    if(added&2) {
                    ...
                    if(added&8) fuck+=2*a[i][1];
                    dp[i+1][nwMask]=max(dp[i+1][nwMask], fuck);
                }
            }
        }
        F0R(i, n+1) {
