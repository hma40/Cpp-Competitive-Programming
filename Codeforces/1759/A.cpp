...
signed main() {
    ...
    while(t--) {
        ...
        if(s[0]=='Y') {
            F0R(i, s.size()) {
                if(i%3==0 && s[i]!='Y') ok=false;
                if(i%3==1 && s[i]!='e') ok=false;
                if(i%3==2 && s[i]!='s') ok=false;
            }
        } else if(s[0]=='e') {
            F0R(i, s.size()) {
                if(i%3==0 && s[i]!='e') ok=false;
                if(i%3==1 && s[i]!='s') ok=false;
                if(i%3==2 && s[i]!='Y') ok=false;
            }
        } else if(s[0]=='s') {
            F0R(i, s.size()) {
                if(i%3==0 && s[i]!='s') ok=false;
                if(i%3==1 && s[i]!='Y') ok=false;
                if(i%3==2 && s[i]!='e') ok=false;
            }
        }
        else ok=false;
        if(ok) cout << "YES" << endl;
