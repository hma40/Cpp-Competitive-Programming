...
signed main() {
    ...
    while(t--) {
        ...
        if(sum.size()<=1 || diff.size()<=1) {
            cout << "YES" << endl;
        } else if(diff.size()==2 && (*diff.begin())+1==(*diff.rbegin())) {
            cout << "YES" << endl;
        } else if(sum.size()==2 && (*sum.begin())+1==(*sum.rbegin())) {
            cout << "YES" << endl;
        } else if(blacks==4) {
            bool f = false;
            F0R(i, n-1) F0R(j, n-1) {
                if(v[i][j]=='#' && v[i][j+1]=='#' && v[i+1][j]=='#' && v[i+1][j+1]=='#') f=true;
            }
            if(f) cout << "YES" << endl;
            else cout << "NO" << endl;
        } else {
