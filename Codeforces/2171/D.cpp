...
signed main() {
    ...
    while(t--) {
        ...
        cin >> n;
        vt<int> a(n);
        set<int> in_connected, in_unconnected;
        F0R(i, n) {
            cin >> a[i];
            in_unconnected.insert(a[i]);
        }
        in_unconnected.erase(a[0]);
        in_connected.insert(a[0]);
        vt<pair<int,int>> v;
        F0R(i, n) {
            // cout << i << endl;
            if(in_unconnected.count(a[i]) && in_connected.size() && (*in_connected.rbegin())>a[i]) {
                v.add({a[i],(*in_connected.rbegin())});
            }
            // cout << i << endl;
            while(in_unconnected.size() && (*in_unconnected.rbegin())>a[i]) {
                v.add({a[i], (*in_unconnected.rbegin())});
                in_connected.insert(*in_unconnected.rbegin());
                in_unconnected.erase(*in_unconnected.rbegin());
            }
            in_unconnected.erase(a[i]);
            in_connected.erase(a[i]);
        }
        if(v.size()==n-1) {
            cout << "YES" << endl;
            // trav(x, v) cout << x.f << " " << x.s << endl;
        } else {
            cout << "NO" << endl;
        }
    }
