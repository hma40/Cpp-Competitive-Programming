...
signed main() {
    ...
    while(t--) {
        ...
        vt<int> ad, sub;
        /*
        assuming alice does operation on
        (i,j)
        suppose i and j are different parity
        suppose i is even
        then score = score-a[i]-a[i]+a[j]+a[j]+abs(i-j)
        */
        int ans = 0;
        int best = -inf;
        F0R(i, n) {
        ...
        int evenMin = inf, oddMax = -inf;
        F0R(i, n) {
            if(i&1) {
                best=max(best, 2*a[i]+i-evenMin);
                oddMax=max(oddMax, 2*a[i]-i);
            } else {
                best=max(best, oddMax-2*a[i]+i);
                evenMin=min(evenMin, 2*a[i]+i);
            }
        }
        int extra = n-1;
        if(n%2==0) extra--;
        cout << ans+max(best, extra) << endl;
    }
    return 0;
}