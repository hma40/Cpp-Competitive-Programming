...
struct SegTree {
    ...
    void set(int pos, int x) {
        ...
        for(pos/=2; pos; pos/=2) {
            ...
            sum[pos]=sum[pos*2]+sum[2*pos+1];
            tree[pos]=max({tree[2*pos], tree[2*pos+1], suff[2*pos]+pref[2*pos+1]});
            pref[pos]=max(pref[2*pos], sum[2*pos]+pref[2*pos+1]);
            suff[pos]=max(suff[2*pos+1], suff[2*pos]+sum[2*pos+1]);
        ...
        int rangeQuery(int a, int b) {
            ...
            b+=n;
            while(a<=b) {
                if(a%2==1) ans=max(ans,mx[a++]);
                if(b%2==0) ans=max(ans,mx[b--]);
                a/=2;
                b/=2;
            }
            return ans;
        ...
        int get(int n) {
            if(tree[1]==0) return rangeQuery(0, n-1);
            else return tree[1];
        }
};
signed main() {
