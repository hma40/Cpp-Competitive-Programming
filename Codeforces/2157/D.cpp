...
signed main() {
    ...
    while(t--) {
        ...
        auto sum = [&](int lr, int rr)->int{
            return pref[rr+1]-pref[lr];
        };
        FOR(negDiff, 1, n+1) {
            int max_pairs = (n-negDiff)/2;
            int lo = 0, hi = max_pairs;
            /*
            a[n-1]+a[n-2]+...+a[n-i]-a[0]-a[1]-...-a[i-1]+negDiff*i-a[i]-a[i+1]-...-a[i+negDiff-1]

            when i increases:
            +a[n-i-1]-a[i]+a[i]-a[i+negDiff]=a[n-i-1]-a[i+negDiff]
            */
            while(lo+3<hi) {
                int mid = (lo+hi)/2;
                int delta = a[n-mid-1]-a[mid+negDiff];
                if(delta>0) lo=mid;
                else hi=mid;
            }
            FOR(i, lo, hi+1) {
                ans=max(ans, sum(n-i, n-1)-sum(0, i-1)+negDiff*l-sum(i, i+negDiff-1));
            }
        }
        FOR(posDiff, 1, n+1) {
            int max_pairs = (n-posDiff)/2;
            int lo = 0, hi = max_pairs;
            /*
            a[n-1]+a[n-2]+...+a[n-i]-a[0]-a[1]-...-a[i-1]-negDiff*r+a[n-i-1]+a[n-i-2]+...+a[n-i-posDiff]

            when i increases:
            +a[n-i-1]-a[i]-a[n-i-1]+a[n-i-posDiff-1]=a[n-i-posDiff-1]-a[i]
            */
            while(lo+3<hi) {
                int mid = (lo+hi)/2;
                int delta = a[n-mid-posDiff-1]-a[lo];
                if(delta>0) lo=mid;
                else hi=mid;
            }
            FOR(i, lo, hi+1) {
                ans=max(ans, sum(n-i, n-1)-sum(0, i-1)-posDiff*r+sum(n-i-posDiff, n-i-1));
            }
        }
        cout << ans << endl;
    }
