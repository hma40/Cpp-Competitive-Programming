...
signed main() {
    ...
    cout << n-1 << endl;
    set<int> need;
    FOR(i, 1, n) need.insert(i);
    auto DO = [&](int x)->void{
        int bruh = n/B*B+x;
        while(bruh>0) {
            if(bruh<n) {
                cout << bruh << " " << B/init << endl;
            }
            need.erase(bruh);
            bruh-=B;
        }
    };
    while(B<=init*init*init) {
        for(int i = B/init; i < B; i+=B/init) {
            DO(i);
        }
        B*=init;
    }
    for(auto x = need.begin(); x!=need.end(); ++x) {
        cout << *x << " " << min(B/init, n-(*x)) << endl;
    }
    return 0;
}