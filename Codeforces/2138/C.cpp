// https://codeforces.com/contest/1856/submission/217308610 with dynamic_bitset<__uint128_t>
...
signed main() {
    ...
    while(t--) {
        ...
        auto dfs = [&](auto self, int node, int depth)->void{
            counts[depth]++;
            if(chil[node].size()==0) smallest=min(smallest, depth);
            trav(x, chil[node]) {
                self(self, x, depth+1);
            }
        };
        ...
        F0R(i, n) while(wCount[i]>=3) {
            if(2*i>=wCount.size()) return 0;
            // assert(2*i<counts.size());
            wCount[2*i]++;
            wCount[i]-=2;
        }
        bitset<200005> dp;
        ...
        F0R(i, k+1) {
            if(dp[i]) {
                int other = nodes-i;
                if(other<=n-k) found=true; 
            }
        }
        if(found) cout << smallest+1 << endl;
