...
signed main() {
    ...
    while(t--) {
        ...
        map<int,int> mp;
        F0R(lp, n) {
            while(rp<n-1 && mp.size()<k) {
                rp++;
                mp[a[rp]]++;
            }
            if(mp.size()==k) minInd[lp]=rp;
            mp[a[lp]]--;
            if(mp[a[lp]]==0) mp.erase(a[lp]);
        }
        ...
        F0R(i, n) {
            if(minInd[i]==inf) continue;
            int mx = min(r, maxInd[i]-i+1);
            int mn = max(l, minInd[i]-i+1);
            ans+=max(0LL, mx-mn+1);
            // cout << i << " " << ans << endl;
        }
        cout << ans << endl;
