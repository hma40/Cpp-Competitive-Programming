...
signed main() {
    ...
    while(t--) {
        ...
        while(lo+1<hi) {
            ...
            int moves = d, rests = mid-d;
            int health = h+rests;
            int large = moves%(rests+1);
            int small = rests+1-large;
            int smallAmt = moves/(rests+1);
            int smallCost = smallAmt*(smallAmt+1)/2;
            int largeCost = (smallAmt+1)*(smallAmt+2)/2;
            // cout << mid << " " << small << " " << large << " " << smallAmt << " " << smallCost << " " << largeCost << endl;
            if(smallCost*small+largeCost*large<health) hi=mid;
            else lo=mid;
        }
        cout << hi << endl;
    }
