...
signed main() {
    ...
    while(t--) {
        ...
        cin >> n >> s;
        vt<int> apos, bpos;
        F0R(i, n) if(s[i]=='a') apos.add(i); else bpos.add(i);
        ...
        int mid = apos[(apos.size()-1)/2];
        int ind = (apos.size()-1)/2;
        F0R(i, apos.size()) {
            int correct = (i-ind)+mid;
            aans+=abs(apos[i]-correct);
        }
        mid = bpos[(bpos.size()-1)/2];
        // cout << bpos << endl;
        ind = (bpos.size()-1)/2;
        // cout << mid << " " << ind << endl;
