...
struct SegTree {
    ...
    void pointSet(int index, int val) {
        // cout << "SETTING " << val << " TO " << index << endl;
...
signed main() {
    ...
    while(t--) {
        ...
        SegTree st(n+5);
        F0R(i, n) {
            int most = st.rangeQuery(0, ib[a[i]]);
            st.pointSet(ib[a[i]]+1, max(st.rangeQuery(ib[a[i]]+1,ib[a[i]]+1),most));
            st.rangeAdd(0, ib[a[i]], p[a[i]]);
        }
        cout << st.rangeQuery(0, n) << endl;
    }   
    return 0;
}