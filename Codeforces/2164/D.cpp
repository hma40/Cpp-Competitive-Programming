...
signed main() {
    ...
    while(t--) {
        ...
        while(lo+1<hi) {
            ...
            F0R(i, n) {
                st[s[i]-'a'].insert(i);
                //need where[i] to be smallest >= than where[i-1]
                int minReq = 0;
                if(i!=0) minReq = where[i-1];
                if(st[ss[i]-'a'].lower_bound(minReq)==st[ss[i]-'a'].end()) {
                    ok=false;
                    break;
                }
                where[i]=*(st[ss[i]-'a'].lower_bound(minReq));
                if(i>=mid) st[s[i-mid]-'a'].erase(i-mid);
                // cout << mid << " " << i << " " << where << endl;
            }
            if(ok) hi=mid;
            else lo=mid;
        }
        ...
        F0R(i, n) {
            ...
            where[i]=*(st[ss[i]-'a'].lower_bound(minReq));
            if(i>=hi) st[s[i-hi]-'a'].erase(i-hi);
            // cout << mid << " " << i << " " << where << endl;
        }
        // cout << where << endl;
        vt<int> actual(n);
