...
signed main() {
    ...
    while(t--) {
        ...
        sort(begin(nonzero),end(nonzero));
        int ans = 0;
        trav(x, nonzero) {
            if((*sword.rbegin())>=x.f) {
                ans++;
                int bruh = *(sword.lower_bound(x.f));
                sword.erase(sword.lower_bound(x.f));
                sword.insert(max(bruh,x.s));
            }
        }
        trav(x, zero) {
            if(sword.size() && (*sword.rbegin())>=x) {
                ans++;
                sword.erase(sword.lower_bound(x));
            }
        }
        cout << ans << endl;
    }
