    ...
    while(t--) {
        ...
        */
        ...
        for(int sec = 2; sec <= n; sec+=2) {
            c+=(n-sec+1)*bexpo(bexpo(m, sec/2-1), mod-2);
            c%=mod;
        }
        for(int fir = 1; fir <= n; fir+=2) {
            int x = (n-fir+1)*(n-1)%mod*bexpo(m, (fir+1)/2+n-fir-1)%mod;
            int here = x*c%mod*2%mod*bexpo(n-1, mod-2)%mod;
            ans+=here;
        }
        for(int fir = 2; fir <= n; fir+=2) {
            int x = (n-fir+1)*(n-1)%mod*bexpo(m, (fir+1)/2+n-fir-1)%mod;
            int here = x*c%mod*bexpo(n-1, mod-2)%mod;
            ans+=here;
        }
        c = 0;
        for(int sec = 1; sec <= n; sec+=2) {
            c+=(n-sec+1)*bexpo(bexpo(m, sec/2), mod-2);
            c%=mod;
        }
        // return 0;

        for(int fir = 1; fir <= n; fir+=2) {
            int x = (n-fir+1)*(n-1)%mod*bexpo(m, (fir+1)/2+n-fir)%mod;
            int here = x*c%mod*bexpo(n-1, mod-2)%mod;
            ans+=here;
        }
        vt<int> v(n+1);
        for(int i = 1; i <= n; i++) {
            ...
            v[i]+=bexpo(m, (i+1)/2-i);
            v[i]%=mod;
            if(i+2<=n) v[i+2]=v[i]+bexpo(m, (i+1)/2-i);
        }
        // return 0;
        FOR(mx, 1, n+1) {
            int M = n+1-mx;
            int temp = M*v[mx]%mod*bexpo(m, (mx+1)/2-mx+n);
            ans-=temp;
            ans%=mod;
            ans+=mod;
            ans%=mod;
        ...
        */
        FOR(larger, 1, n+1) {
            ...
            int here = choices*bexpo(m, n-larger+(larger+1)/2)%mod*(n-larger+1);
            // cout << "line 210 " << larger << " " << here << endl;
            ans+=here;
            ans%=mod;
            ans+=mod;
            ans%=mod;
